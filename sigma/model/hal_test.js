...
test("Resource model", function(){
	stop();
	var hal_sample = Test.HALSample.getRoot(1)
	hal_sample.then(function(pr){
...
test("Resource model overrides", function(){
	stop();
	Sigma.Model.HAL.Resource
	(
...
test("Fetch", function(){
	...
	.then(	function(pr)
		{
			//hay que implementar la logica de colecciones
			pr.links.attr('multiples.0').fetch()
